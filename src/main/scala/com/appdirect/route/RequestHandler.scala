...
import com.appdirect.domain.common.Error
import com.appdirect.service.Aggregator
...
import akka.actor._
import AggregateRoot.{Removed, Uninitialized}
...
object RequestHandler {
  ...
  case class RegisterRequestActor[S <: AggregateRoot.State](
      r: RequestContext, 
      target: ActorRef, 
      message: Aggregator.Command)(implicit tag: ClassTag[S]) extends RequestHandler {
  ...
  case class DeleteRequestActor(
      r: RequestContext, 
      target: ActorRef, 
      message: Aggregator.Command) extends RequestHandler {
    override def processResult: Receive = {
 ...
 * Immediately after instantiation it sends a command to specific [[Aggregator]].
 * It then awaits response from [[Aggregator]] or handles exception or timeout if one of them occurred.
...
trait RequestHandler extends Actor with ActorLogging with Json4sSupport {
  ...
  def target: ActorRef
  def message: Aggregator.Command
...
trait RequestHandlerCreator {
  ...
  def handleRegister[S <: AggregateRoot.State](
      r: RequestContext, 
      target: ActorRef, 
      message: Aggregator.Command)(implicit tag: ClassTag[S]) =
